"""GameLoot (gameloot.in) Gaming, PC & Hardware Scraper.
...
class GameLootScraper(BaseScraper):
    platform: str = "gameloot"
    ...
    async def scrape(self, url: str) -> ScrapeResult:
        ...
        # Check explicit GameLoot stock indicators (e.g. .stock.out-of-stock, button or badge)
        stock_badge = soup.select_one(".stock, .out-of-stock, .badge-out-of-stock")
        if stock_badge:
            badge_text = stock_badge.get_text(" ", strip=True).lower()
            if "sold out" in badge_text or "out of stock" in badge_text:
                in_stock = False
